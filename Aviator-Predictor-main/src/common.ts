...
export abstract class jsSHABase<StateT, VariantT> {
  ...
  protected constructor(variant: any, inputFormat: any, options?: any) {
    ...
    if (isNaN(this.numRounds) || this.numRounds !== parseInt(String(this.numRounds), 10) || this.numRounds < 1) {
  ...
  protected _getHMAC(): number[] {
    ...
    const out = this.finalizeFunc(
      first,
      this.outputBinLen,
      this.variantBlockSize,
      this.roundFunc(this.keyWithOPad, this.newStateFunc(this.shaVariant)),
      this.outputBinLen
    );
